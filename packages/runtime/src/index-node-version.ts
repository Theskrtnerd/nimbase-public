...
export async function indexNodeVersion(args: {
  ...
  // Frontmatter contains volatile server-owned fields (notably updatedAt) and
  // access/provenance metadata. It is useful for governance but harmful to
  // retrieval: embedding it makes otherwise identical notes change vectors on
  // every write and lets metadata dominate short content. Index note content
  // only; datasets continue to use their semantic summary.
  const indexable =
    kind === "dataset" ? (summary ?? "") : parseOkf(body).content;
  const chunks = chunkMarkdown(indexable);
