 ...
 *
 * Loaded through the local, integrity-verifying artifact runtime. It is
 * self-contained — no `eval`, no `new Function`, no dynamic `import()` — so it
 * runs under the share sandbox without `unsafe-eval`.
 */
 ...
 */
export const ARTIFACT_MERMAID_HEAD = `<script src="${artifactRuntimeUrl("mermaid")}"></script>
<script>
(function () {
